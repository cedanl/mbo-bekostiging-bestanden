...
def test_export_frames_parquet_creates_files(frames, tmp_path):
    export_frames(frames, tmp_path)
    assert len(list(tmp_path.glob("*.parquet"))) == len(frames)


def test_export_frames_parquet_creates_output_dir(frames, tmp_path):
    target = tmp_path / "nieuw" / "pad"
    export_frames(frames, target)
    assert target.exists()


def test_export_frames_parquet_filenames_match_recordtypes(frames, tmp_path):
    export_frames(frames, tmp_path)
    written = {f.stem for f in tmp_path.glob("*.parquet")}
    assert written == set(frames.keys())


def test_export_frames_parquet_preserves_types(frames, tmp_path):
    export_frames(frames, tmp_path)
...
def test_export_frames_parquet_row_counts(frames, tmp_path):
    export_frames(frames, tmp_path)
...
def test_export_frames_csv_creates_files(frames, tmp_path):
    export_frames(frames, tmp_path, fmt="csv")
    assert len(list(tmp_path.glob("*.csv"))) == len(frames)


def test_export_frames_csv_filenames_match_recordtypes(frames, tmp_path):
    export_frames(frames, tmp_path, fmt="csv")
    written = {f.stem for f in tmp_path.glob("*.csv")}
    assert written == set(frames.keys())


def test_export_frames_csv_readable(frames, tmp_path):
    export_frames(frames, tmp_path, fmt="csv")
...
def test_export_frames_invalid_fmt_raises(frames, tmp_path):
    with pytest.raises(ValueError, match="Onbekend formaat"):
        export_frames(frames, tmp_path, fmt="xlsx")  # type: ignore
...
def test_export_frames_no_format_mixing(frames, tmp_path):
    export_frames(frames, tmp_path / "parquet", fmt="parquet")
    export_frames(frames, tmp_path / "csv", fmt="csv")
